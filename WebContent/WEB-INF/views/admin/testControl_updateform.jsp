...
<head>
    <title>Udemy - UpdCurriculum</title>
    <meta charset="UTF-8">
...
<body>
    <div class="position-absolute d-flex" style="left:50%;top:50%;transform: translate(-50%,-50%);box-shadow: 0px 0px 10px rgba(0,0,0,.2);">
        <iframe src="${link}/pdf_for_test/${test.pdf}" width="100%" height="600px" class="border output">
      </iframe>
         <form:form class="bg-white" action="testUpdform.htm?id=${courseid}&testid=${test.id}" method="POST" enctype="multipart/form-data" style=" height:600px;width: 1200px;">
            <a class="btn my-3" href="curriculum.htm?id=${courseid}"><img src="${link}/img/Arrow 1.png" alt="back" width="30px"> Back to previous page </a>
            ...
            <div class="container w-75">
                 <div class="form-control d-flex">
                    ...
                    <input type="text" name="name" class="w-75" value="${test.name }">
                </div>

                <div class="err">${name_err }</div>
                <div class="form-control d-flex">
                    <label class="w-25">Time</label>
                    <input type="number" name="time" class="w-75" value="${test.time}" required>
                </div>
                <div class="err">${time_err }</div>
                <div class="form-control d-flex">
                    <label class="w-25">Question</label>
                    <input type="number" name="ques" class="w-75" value="${test.question}" required>
                </div>
                <div class="err">${question_err }</div>
                <div class="form-control d-flex">
                    <label class="w-25">Answer</label>
                    <input type="text" name="answer" class="w-75" value="${test.answer}" required>
                </div>
                ...
                <div class="form-control d-flex">
                    <label class="w-25">Old Pdf</label>
                    <span>${test.pdf }</span>
                </div>
